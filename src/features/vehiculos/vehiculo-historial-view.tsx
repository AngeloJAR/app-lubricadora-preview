...
import type { HistorialVehiculoOrden } from "@/types";
import { getEstadoClasses, getEstadoLabel } from "@/utils/orden-status";

type VehiculoHistorialViewProps = {
  historial: HistorialVehiculoOrden[];
};

export function VehiculoHistorialView({
  historial,
}: VehiculoHistorialViewProps) {
  return (
    <Card title="Historial del vehículo">
      {historial.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-300 p-6 text-center text-gray-500">
          Este vehículo todavía no tiene historial de servicios.
        </div>
      ) : (
          ...
          {historial.map((orden) => (
            ...
            >
              ...
              <div className="mt-4 grid gap-2 md:grid-cols-3">
                ...
                <div>
                  <p className="text-sm text-gray-500">Próximo km</p>
                  <p className="font-medium">
